...
useEffect(() => {
  if (sidebarRef.current && navbarRef.current) {
    ...
    navbarRef.current.style.transition = 'width 0.3s ease, left 0.3s ease';
  }
}, []);
  ...
  return (
    <>
      ...
      role="button" className={cn("h-6 w-6 text-muted-foreground rounded-sm hover:bg-neutral-300 dark:hover:bg-neutral-600 absolute top-3 right-2 opacity-0 group-hover/sidebar:opacity-100 transition",
        ...
        onClick={resetWidth}
        className="opacity-0 group-hover/sidebar:opacity-100 transition cursor-ew-resize absolute  top-0 w-1 bg-primary/10 h-full right-0 "
        />
      </aside>
